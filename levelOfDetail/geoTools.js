...
export function georeferenceObj(obj, terrain) {
  if (obj && terrain && terrain.isGeoreferenced) {
    ...
    let elev =
      orientation.Elevation || orientation.Altitude || orientation.altitude;
...
export function rotateQuaternionFromECEF(quaternion, terrain, lat, lon) {
  ...
  q_.multiply(new THREE.Quaternion().setFromRotationMatrix(r).invert());
...
export function isInBounds(lat, lon, bounds) {
  if (
    ...
    if (
      ...
      if (lon < east && lon > west && lat < north && lat > south) return true;
    }
  }
  return false;
}
